/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   colors.h                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ozamora- <ozamora-@student.42madrid.com    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/08 13:16:14 by ozamora-          #+#    #+#             */
/*   Updated: 2025/06/08 14:06:23 by ozamora-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef COLORS_H
# define COLORS_H

# define BLACK 0x000000FF
# define WHITE 0xFFFFFFFF
# define GREY 0x808080FF
# define LIGHT_GREY 0xC0C0C0FF
# define DARK_GREY 0x404040FF

# define RED 0xFF0000FF
# define LIGHT_RED 0xFF7F7FFF
# define DARK_RED 0x8B0000FF

# define GREEN 0x00FF00FF
# define LIGHT_GREEN 0x90EE90FF
# define DARK_GREEN 0x006400FF

# define BLUE 0x0000FFFF
# define LIGHT_BLUE 0xADD8E6FF
# define DARK_BLUE 0x00008BFF

# define TURQUOISE 0x40E0D0FF
# define LIGHT_TURQUOISE 0xAFEEEEFF
# define DARK_TURQUOISE 0x008B8BFF

# define YELLOW 0xFFFF00FF
# define LIGHT_YELLOW 0xFFFFE0FF
# define DARK_YELLOW 0x808000FF

# define ORANGE 0xFFA500FF
# define LIGHT_ORANGE 0xFFDAB9FF
# define DARK_ORANGE 0xFF8C00FF

# define PURPLE 0x800080FF
# define LIGHT_PURPLE 0xDDA0DDFF
# define DARK_PURPLE 0x4B0082FF

# define PINK 0xFFC0CBFF
# define LIGHT_PINK 0xFFB6C1FF
# define DARK_PINK 0xFF69B4FF

# define BROWN 0xA52A2AFF
# define LIGHT_BROWN 0xD2B48CFF
# define DARK_BROWN 0x8B4513FF

#endif